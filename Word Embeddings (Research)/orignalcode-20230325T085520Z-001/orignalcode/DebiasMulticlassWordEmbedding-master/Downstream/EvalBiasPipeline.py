...
def biasEvalPipeline(biasWvsPath, debiasWvsPath, nerPath, dataTarget, logFile=None, verbose=False, results=True):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if(val_loss < bestValLoss and not start):
            torch.save(posModel.state_dict(), "/content/drive/MyDrive/orignalcode/DebiasMulticlassWordEmbedding-master/Downstream/models/savedModels/model.m")
            bestValLoss = val_loss
        start = False

    if(verbose):
        print("Found best case validation loss to be " + str(bestValLoss) + "\n\t... Loading saved model and testing")
    posModel.load_state_dict(torch.load("/content/drive/MyDrive/orignalcode/DebiasMulticlassWordEmbedding-master/Downstream/models/savedModels/model.m"))
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if(verbose):
            print("Epoch #{} - \n\tVal Loss: {:.6f} \n\tVal Precision {:6f} \n\tVal Recall {:6f} \n\tVal Macro F1 {:6f}".format(epoch, val_loss, precision, recall, f1))
        if(val_loss < bestValLoss and not start):
            torch.save(debias_posModel.state_dict(), "/content/drive/MyDrive/orignalcode/DebiasMulticlassWordEmbedding-master/Downstream/models/savedModels/debiased_model.m")
            bestValLoss = val_loss
        start = False

    if(verbose):
        print("Found best case validation loss to be " + str(bestValLoss) + "\n... Loading saved model and testing")
    debias_posModel.load_state_dict(torch.load("/content/drive/MyDrive/orignalcode/DebiasMulticlassWordEmbedding-master/Downstream/models/savedModels/debiased_model.m"))
