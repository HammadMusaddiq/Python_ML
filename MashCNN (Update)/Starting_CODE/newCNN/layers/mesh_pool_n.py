...
class MeshPool(nn.Module):
    ...
    def __pool_edge(self, mesh, edge_id, mask, edge_groups):
        if self.has_boundaries(mesh, edge_id):
            ...
            mask[edge_id] = False
            self.__remove_group(mesh, edge_groups, edge_id)
            mesh.edges_count -= 1
            return True
        else:
            return False
    ...
    def __pool_side(self, mesh, edge_id, mask, edge_groups, side):
        info = self.__get_face_info(mesh, edge_id, side)
        ...
        self.__union_groups(mesh, edge_groups, key_b, key_a)
        self.__union_groups(mesh, edge_groups, edge_id, key_a)
        mask[key_b] = False
        self.__remove_group(mesh, edge_groups, key_b)
    ...
    def __get_invalids(self, mesh, edge_id, edge_groups, side):
        ...
        shared_items = self.__get_shared_items(other_keys_a, other_keys_b)
        if len(shared_items) == 0:
            ...
            self.__redirect_edges(mesh, edge_id, side, update_key_a, update_side_a)
            self.__redirect_edges(mesh, edge_id, side + 1, update_key_b, update_side_b)
            self.__redirect_edges(mesh, update_key_a, self.__get_other_side(update_side_a), update_key_b, self.__get_other_side(update_side_b))
            self.__union_groups(mesh, edge_groups, key_a, edge_id)
            self.__union_groups(mesh, edge_groups, key_b, edge_id)
            self.__union_groups(mesh, edge_groups, key_a, update_key_a)
            self.__union_groups(mesh, edge_groups, middle_edge, update_key_a)
            self.__union_groups(mesh, edge_groups, key_b, update_key_b)
            self.__union_groups(mesh, edge_groups, middle_edge, update_key_b)
